...
public class BatteryObject implements IBatteryIOObject, MjReconfigurator.IConfigurableBatteryObject {
	...
	public double getEnergyRequested() {
		...
		try {
			return bounds(batteryData.maxCapacity() - energyStored.getDouble(obj),
					batteryData.minimumConsumption(), batteryData.maxReceivedPerCycle());
		} catch (IllegalAccessException e) {
			BCLog.logger.warn("can't get energy requested");
		}
		return 0;
	}
	...
	public double addEnergy(double mj, boolean ignoreCycleLimit) {
		try {
			...
			BCLog.logger.warn("can't add energy");
		}
		return 0;
	}
	...
	public double extractEnergy(double mj, boolean ignoreCycleLimit) {
		try {
			...
			BCLog.logger.warn("can't extract energy");
		}
		return 0;
	}
	...
	public double getEnergyStored() {
		try {
			...
			BCLog.logger.warn("can't get return energy stored");
			return 0;
		}
	}
	...
	public void setEnergyStored(double mj) {
		try {
			...
			BCLog.logger.warn("can't set energy stored");
